...
<style>
    ...
    [data-testid="stMetric"] { background-color: #1E1E1E; border: 1px solid #333333; padding: 15px; border-radius: 12px; }
    ...
    .stTabs [data-baseweb="tab"] { background-color: #1E1E1E; border-radius: 8px; color: #A0A0A0; padding: 10px 16px; }
...
if st.sidebar.button("💾 Salvar Registro"):
    ...
    st.sidebar.success("Lançamento salvo na planilha!")
    st.rerun()
...
if not df.empty:
    ...
    df["Clientes"] = pd.to_numeric(df["Clientes"], errors="coerce").fillna(0)
    df["Comissao_Pct"] = pd.to_numeric(df["Comissao_Pct"], errors="coerce").fillna(0)
...
with aba1:
    ...
    c5.metric("Atendimentos", f"{int(total_clientes)} clientes")
